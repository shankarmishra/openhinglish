...
> **Status as of 2026-06:** Engine is early-functional — 51 unit tests passing, ~1,500+
> lexicon entries, deterministic context disambiguator (V3) implemented, REST API + web UI
> + CLI shipped, honest 59-sentence benchmark at 0.92 display EM / 0.92 TTS EM. Not production-ready; lexicon
...
| **V1 "Usable Hindi+English"** | Scale lexicon; real-world coverage | **IN PROGRESS** | ~1,500+ entries now (roman_hindi ~500, verbs, function words, English, names, brands); target ~10k via Dakshina; bench at 59 sentences, display EM 0.92 / TTS EM 0.92 — real gaps remain (multi-word addresses, code-switch boundaries, multi-word abbreviations) |
...
| **V5 "The Standard"** | IndianTTSBench public leaderboard, governance | **STARTED** | `IndianTTSBench-mini` has grown to 59 gold sentences across 11 categories; honest display EM 0.92 / TTS EM 0.92 (the earlier 1.000 figure was over 6 cherry-picked rows and has been retired); benchmark exposes real engine gaps (roman-Hindi long-tail vocabulary, code-switch). NOT started: public leaderboard hosting, community governance structure |
...
- Bench-mini reported 1.000 n-best coverage over its 6 rows (trivially true: rows were
  ...
  honest 59-sentence benchmark at 0.92 display EM / 0.92 TTS EM introduced in V5 work).
...
**Current state (2026-06):** `IndianTTSBench-mini` has grown to **59 gold sentences**
across 11 categories. The exact-match score is honestly reported as **0.92 display / 0.92 TTS**
— the previous 1.000 figure (over 6 cherry-picked rows) has been retired as misleading.
The benchmark exposes real engine gaps: long-tail roman-Hindi vocabulary (worst category), code-switch
...
|---|---|---|---|---|---|---|
| Lexicon entries (Roman-Hindi) | ~13 seed → **~470+ now** (all lexicons ~1,500+) | 10 000+ | 10 000+ (stable) | same | same | same |
...
| Bench size (gold rows) | 6 seed → **59 now** | 300+ | 300+ + 6×50 per lang | 300+ + adversarial | same | public leaderboard |
| n-best coverage @k=3 (Hindi+Hinglish bench) | ~~1.000 (6 rows)~~ → **0.92 display EM (59 rows)** | ≥ 85% | ≥ 85% Hindi; ≥ 70% new langs | ≥ 90% Hindi | same | same |
| Display-accuracy @k=1 | **0.92 EM (honest, 59 rows)** | ≥ 80% | ≥ 80% Hindi | ≥ 85% | same | public |
| TTS-accuracy @k=1 | **0.92 EM (honest, 59 rows)** | ≥ 75% | ≥ 75% Hindi | ≥ 82% | same | public |
...
~1,500+ entries and the honest 59-sentence benchmark shows 0.92 display EM / 0.92 TTS EM. That is real
